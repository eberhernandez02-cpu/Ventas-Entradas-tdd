...
class SistemaVentas:
    ...
    def registrar_evento(self, evento: Evento) -> None:
        ...
        self._eventos[evento.nombre] = evento
    ...
    def vender_entradas(self, nombre_evento: str, cantidad: int) -> float:
        ...
        """
        evento = self._eventos.get(nombre_evento)
        if evento is None:
            raise VentaError("El evento no se encuentra registrado.")
        if cantidad <= 0:
            raise VentaError("La cantidad de entradas debe ser mayor que cero.")
